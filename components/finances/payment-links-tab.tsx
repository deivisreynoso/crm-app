  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-body-muted">
        Payment links are created from an invoice. Open an invoice and use Payments &amp; collection to generate one.
      </p>
              ...
              {rows.map((link) => (
                <DataTableRow key={link.id}>
                  ...
                  <DataTableCell>
                    {link.invoice ? (
                      <Link
                        href={`/finances/invoices/${link.invoice.id}`}
                  ...
                  <DataTableCell>
                    {link.invoice?.quote?.quote_reference
                      ? `Quote ${link.invoice.quote.quote_reference}`
                      : "—"}
                  </DataTableCell>
