...
type Tab = "all" | "templates" | "branding" | "products";
...
function QuotesPageInner() {
  ...
  const { data: ctx } = useWorkspaceContext();
  const canManageQuotes = ctx?.canManage ?? false;
  ...
  const tabs: { id: Tab; label: string; manageOnly?: boolean }[] = [
    { id: "all", label: q?.allQuotes ?? "All quotes" },
    { id: "templates", label: q?.templates ?? "Templates" },
    { id: "branding", label: dict.settings?.quoteBranding ?? "Branding", manageOnly: true },
    { id: "products", label: dict.settings?.quoteServices ?? "Products", manageOnly: true },
  ];
  ...
  return (
    <div className="space-y-6 w-full">
      ...
      <nav className="flex flex-wrap gap-4 border-b border-[var(--card-border)]">
        {tabs
          .filter((t) => !t.manageOnly || canManageQuotes)
          .map((t) => (
            <button
              key={t.id}
              type="button"
      ...
      {tab === "templates" ? (
        ...
        canManageQuotes ? (
          ...
          <p className="text-sm text-body-muted">Branding is managed by workspace admins.</p>
        )
      ) : tab === "products" ? (
        canManageQuotes ? (
          <QuoteServicesCatalog />
        ) : (
          <p className="text-sm text-body-muted">Products are managed by workspace admins.</p>
        )
      ) : (
        <>
          <QuoteAnalyticsPanel />
            ...
            {isLoading ? (
              ...
              <table className="w-full text-sm">
                <thead className="bg-[var(--background)] border-b border-[var(--card-border)]">
                  <tr>
                    <th className="text-left px-4 py-3 font-medium text-heading">{q?.item}</th>
                    <th className="text-left px-4 py-3 font-medium text-heading">{q?.status}</th>
                    <th className="text-right px-4 py-3 font-medium text-heading">{q?.total}</th>
...
export default function QuotesPage() {
  return (
    <Suspense fallback={<p className="p-6 text-body-muted">Loading quotes…</p>}>
      <QuotesPageInner />
    </Suspense>
  );
}
