...
export async function PATCH(req: NextRequest, context: RouteContext) {
  try {
    ...
    if (parsed.data.grants) {
      ...
      await replaceCustomRoleGrants(supabase, workspaceOwnerId!, id, grants);
    }

    const rows = await listCustomRoles(supabase, workspaceOwnerId!);
    const row = rows.find((r) => r.id === id);
    return NextResponse.json({ data: row });
  } catch (err) {
    console.error("PATCH /api/settings/custom-roles/[id]:", err);
...
export async function DELETE(_req: NextRequest, context: RouteContext) {
  try {
    ...
    console.error("DELETE /api/settings/custom-roles/[id]:", err);
    return NextResponse.json({ error: "Internal Server Error" }, { status: 500 });
  }
}
